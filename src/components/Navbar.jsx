...
const Navbar = () => {
  ...
  const navLinks = [
    { name: "Home", href: "#home" }, // Added Home button
      ...
      <AnimatePresence>
        {open && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-x-6 top-24 bg-[#0F172A]/95 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 flex flex-col items-center gap-6 md:hidden shadow-2xl"
          >
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                ...
                className="text-xl font-semibold text-slate-200 hover:text-blue-500 transition-colors"
              >
                {link.name}
              </a>
            ))}
